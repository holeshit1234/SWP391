    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (paymentID != 0 && addressID != 0) { //đặt lock
                ...
                /*
                // Create a Redisson client configuration
                Config config = new Config();
                config.useSingleServer().setAddress("redis://localhost:8084");
                // Create a Redisson client instance with the configuration
                RedissonClient redisson = Redisson.create(config);
                // Get a lock object with a given name
                RLock lock = redisson.getLock("myLock");
                // Acquire the lock and perform some work
                lock.lock();
                //-----------------------------------------------------------------
                //Thực thi sau khi đặt lock ---------------------------------------
                */
                ...
                if (error) {
                    //thong bao looi
                    url="erorCheckout.jsp";
                    //------------------------------------------------------
                } else {
                    //add data vào 2 bảng
                    //Order table
                    double shippingFee = 30000;
                    OrderDTO order = new OrderDTO(0, userID, paymentID, addressID, null, totalPrice, shippingFee, 1, false);
