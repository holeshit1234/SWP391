...
public class DeleteCartServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            //get para
            int cartID =0;
            String txtCartID = request.getParameter("txtCartID");
            if (txtCartID != null) {
                cartID = Integer.parseInt(txtCartID.trim());
            }
...
//            UserDetailsDTO currentUserID = (UserDetailsDTO) session.getAttribute("USER");
            ...
            boolean result = dao.deleteCart(cartID);
            if (result) {
                System.out.println("Deleted cart item!");
            }

        } catch (NamingException ex) {
            log("DeleteCartServlet_Naming " + ex.getMessage());
        } catch (SQLException ex) {
            log("DeleteCartServlet_SQL " + ex.getMessage());
        } finally {
